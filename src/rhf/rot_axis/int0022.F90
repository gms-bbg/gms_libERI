submodule(rot_axis_kernels) int0022_impl
contains
  module subroutine int0022(ss_pair, dd_pair, density, fock, res)
    ...
    integer(kind=int64), allocatable :: n00bra(:), n22ket(:)
    real(dp), allocatable :: xint00bra(:), xint22ket(:)
    integer(kind=int64) :: nssbra, nddket
    ...
    real(dp) :: scutssbra, scutddket
    real(dp) :: r12, r34, buff(12), cosg, sing, rcd, rab, acx, acy, acz, tmp
    real(dp) :: d00p, d22p, t_expon_ab, t_inverse_expon_ab, t_inverse_expon_cd, t_alpha, t_beta, t_alpha_cd, t_beta_cd
    real(dp) :: sq, cq, cqx, cqz, aqx, aqx2, aqxy, aqz, qps, acy2
    !for boys function
    integer(kind=int64) :: t_int
    real(dp) ::  zero_m_0(1), zero_m_1(2), zero_m_2(3), zero_m_3(4), zero_m_4(5), boys0, boys1, boys2, boys3, boys4
    real(dp) :: expon_abcd_inverse, t_expon_cd, pqr, pqs, fmt, t_new, t_inverse, rho, t, expt
    !r-integrals
    real(dp) :: r00(5), r01(12), r02(24), r03(20), r04(15), y03
    !eri_value
   ...
   integer(kind=int64) :: ii1,kk1,nij,maxl2,jj1,i2,j2,ijp,nkl,itmp,ijklp,ll2,k2,l2,ii2,jj2,kk2,ik,il,jk,jl,ll1,ijkp,ijk_index,ijkl_index
    ...
    scutssbra = cutoff_schwarz/maxval(dd_pair%xints)
    nssbra = 0
    ...
    scutddket = cutoff_schwarz/maxval(ss_pair%xints)
    nddket = 0
    ...
    if ((nssbra*nddket) .le. nchunksize_int64) nchunksize_int64 = nssbra*nddket
    ntile = int(nssbra*nddket/nchunksize_int64)
    do itile = 1, ntile
      istart = (itile - 1)*nchunksize_int64 + 1
      iend = itile*nchunksize_int64
      if (itile .eq. ntile) iend = nssbra*nddket
 ...
 !$omp shared(res, density, fock, boys_grid_zero, exponent_grid, dd_pair, ss_pair) &
 !$omp shared(nquart_start, nquart_end, xint00bra, xint22ket) &
 !$omp shared(nddket, n00bra, n22ket) &
 ...
 !$omp private(trans, jj1, kk1, i2, j2, ll1, k2, l2, ik, il, jk, jl, ii1, jj2, kk2, ii2, k,i,l)
      do iquart = nquart_start, nquart_end
        ...
        test = xint00bra(ij_tmp)*xint22ket(kl_tmp)
        if (test .gt. cutoff_schwarz) then
          ...
          ! write(*,*) "res%i_s_shl(ish_tmp)", res%i_s_shl(ish_tmp)
          ! write(*,*) "res%i_s_shl(jsh_tmp)", res%i_s_shl(jsh_tmp)

          ish = ii
          jsh = jj
          ksh = kk
          lsh = ll
          ! if(ii.eq.7.and.jj.eq.7.and.kk.eq.6.and.ll.eq.6) then
          ! write(*,*) "ii", ii
          ! write(*,*) "jj", jj
          ! write(*,*) "kk", kk
          ! write(*,*) "ll", ll
  ...
  acz = (res%coord_sh(ksh,1) - res%coord_sh(ish,1))* buff(1) +(res%coord_sh(ksh,2) - res%coord_sh(ish,2))* buff(2) +(res%coord_sh(ksh,3) - res%coord_sh(ish,3))* buff(3)
          ...
          do k = 1, res%contr_num(ksh)*res%contr_num(lsh)
            ...
            do i = 1, res%contr_num(ish)*res%contr_num(jsh)
              ...
              shp_thresh = ss_pair%ismlp(ss_pair%pair_loc(ij) + bra_loop) + dd_pair%ismlp(dd_pair%pair_loc(kl) + ket_loop)
              ...
              if (t .le. t_max) then
                ...
                !    write(*,*) fgrid, boys_grid_zero((4*451*5)+(t_int*5)+5)
                ...
                boys3 = boys3*sqrt(expon_abcd_inverse)*d00p*rho*rho*rho
                boys4 = fmt*sqrt(expon_abcd_inverse)*d00p*rho*rho*rho*rho
              else
              ...
              zero_m_4(1) = zero_m_4(1) + boys4
              zero_m_4(2) = zero_m_4(2) + boys4*pqr
              zero_m_4(3) = zero_m_4(3) + boys4*pqs
              zero_m_4(4) = zero_m_4(4) + boys4*pqr*pqs
              zero_m_4(5) = zero_m_4(5) + boys4*pqs*pqs
            end do
            !r integrals here
            ...
            r00(5) = r00(5) + zero_m_0(1)*y03*y03*(1 - y03)*(1 - y03)*sq
            ! write(*,*) "r00(1)", r00(1) !ok
            ! write(*,*) "r00(2)", r00(2)
            ! write(*,*) "r00(3)", r00(3)
            ! write(*,*) "r00(4)", r00(4)
            ! write(*,*) "r00(5)", r00(5)
            r01(1) = r01(1) + zero_m_1(1)*aqx*t_inverse_expon_cd*sq*t_inverse_expon_cd*y03*0.25_dp
            r01(2) = r01(2) + zero_m_1(1)*acy*t_inverse_expon_cd*sq*t_inverse_expon_cd*y03*0.25_dp
            r01(3) = r01(3) - zero_m_1(2)*t_inverse_expon_cd*sq*t_inverse_expon_cd*y03*0.25_dp
            r01(4) = r01(4) - zero_m_1(1)*aqx*t_inverse_expon_cd*sq*t_inverse_expon_cd*(1 - y03)*0.25_dp
            r01(5) = r01(5) - zero_m_1(1)*acy*t_inverse_expon_cd*sq*t_inverse_expon_cd*(1 - y03)*0.25_dp
            r01(6) = r01(6) + zero_m_1(2)*t_inverse_expon_cd*sq*t_inverse_expon_cd*(1 - y03)*0.25_dp
            r01(7) = r01(7) - zero_m_1(1)*aqx*t_inverse_expon_cd*0.5_dp*sq*y03*y03*(1 - y03)
            r01(8) = r01(8) - zero_m_1(1)*acy*t_inverse_expon_cd*0.5_dp*sq*y03*y03*(1 - y03)
            r01(9) = r01(9) + zero_m_1(2)*t_inverse_expon_cd*0.5_dp*sq*y03*y03*(1 - y03)
            r01(10) = r01(10) + zero_m_1(1)*aqx*t_inverse_expon_cd*0.5_dp*sq*y03*(1 - y03)*(1 - y03)
            r01(11) = r01(11) + zero_m_1(1)*acy*t_inverse_expon_cd*0.5_dp*sq*y03*(1 - y03)*(1 - y03)
            r01(12) = r01(12) - zero_m_1(2)*t_inverse_expon_cd*0.5_dp*sq*y03*(1 - y03)*(1 - y03)
            ! do i = 1,12
            !     write(*,*) "r01", r01(i)
            ! enddo !ok

            r02(1) = r02(1) + (zero_m_2(1)*aqx2 - zero_m_1(1))*t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd*0.125_dp
            r02(2) = r02(2) + (zero_m_2(1)*acy2 - zero_m_1(1))*t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd*0.125_dp
            r02(3) = r02(3) + (zero_m_2(3) - zero_m_1(1))*t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd*0.125_dp
            r02(4) = r02(4) + (zero_m_2(1)*aqxy)*t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd*0.125_dp
            r02(5) = r02(5) - (zero_m_2(2)*aqx)*t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd*0.125_dp
            r02(6) = r02(6) - (zero_m_2(2)*acy)*t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd*0.125_dp
            r02(7) = r02(7) + (zero_m_2(1)*aqx2 - zero_m_1(1))*t_inverse_expon_cd*sq*t_inverse_expon_cd*y03*y03*0.25_dp
            r02(8) = r02(8) + (zero_m_2(1)*acy2 - zero_m_1(1))*t_inverse_expon_cd*sq*t_inverse_expon_cd*y03*y03*0.25_dp
            r02(9) = r02(9) + (zero_m_2(3) - zero_m_1(1))*t_inverse_expon_cd*sq*t_inverse_expon_cd*y03*y03*0.25_dp
            r02(10) = r02(10) + (zero_m_2(1)*aqxy)*t_inverse_expon_cd*sq*t_inverse_expon_cd*y03*y03*0.25_dp
            r02(11) = r02(11) - (zero_m_2(2)*aqx)*t_inverse_expon_cd*sq*t_inverse_expon_cd*y03*y03*0.25_dp
            r02(12) = r02(12) - (zero_m_2(2)*acy)*t_inverse_expon_cd*sq*t_inverse_expon_cd*y03*y03*0.25_dp
            r02(13) = r02(13) - (zero_m_2(1)*aqx2 - zero_m_1(1))*t_inverse_expon_cd*sq*t_inverse_expon_cd*0.25_dp*y03*(1 - y03)
            r02(14) = r02(14) - (zero_m_2(1)*acy2 - zero_m_1(1))*t_inverse_expon_cd*sq*t_inverse_expon_cd*0.25_dp*y03*(1 - y03)
            r02(15) = r02(15) - (zero_m_2(3) - zero_m_1(1))*t_inverse_expon_cd*sq*t_inverse_expon_cd*0.25_dp*y03*(1 - y03)
            r02(16) = r02(16) - (zero_m_2(1)*aqxy)*t_inverse_expon_cd*sq*t_inverse_expon_cd*0.25_dp*y03*(1 - y03)
            r02(17) = r02(17) + (zero_m_2(2)*aqx)*t_inverse_expon_cd*sq*t_inverse_expon_cd*0.25_dp*y03*(1 - y03)
            r02(18) = r02(18) + (zero_m_2(2)*acy)*t_inverse_expon_cd*sq*t_inverse_expon_cd*0.25_dp*y03*(1 - y03)
           r02(19) = r02(19) + (zero_m_2(1)*aqx2 - zero_m_1(1))*t_inverse_expon_cd*sq*t_inverse_expon_cd*0.25_dp*(1 - y03)*(1 - y03)
           r02(20) = r02(20) + (zero_m_2(1)*acy2 - zero_m_1(1))*t_inverse_expon_cd*sq*t_inverse_expon_cd*0.25_dp*(1 - y03)*(1 - y03)
            r02(21) = r02(21) + (zero_m_2(3) - zero_m_1(1))*t_inverse_expon_cd*sq*t_inverse_expon_cd*0.25_dp*(1 - y03)*(1 - y03)
            r02(22) = r02(22) + (zero_m_2(1)*aqxy)*t_inverse_expon_cd*sq*t_inverse_expon_cd*0.25_dp*(1 - y03)*(1 - y03)
            r02(23) = r02(23) - (zero_m_2(2)*aqx)*t_inverse_expon_cd*sq*t_inverse_expon_cd*0.25_dp*(1 - y03)*(1 - y03)
            r02(24) = r02(24) - (zero_m_2(2)*acy)*t_inverse_expon_cd*sq*t_inverse_expon_cd*0.25_dp*(1 - y03)*(1 - y03)
            ! do i = 1,24
            ! write(*,*) "r02", r02(i)
            ! enddo
            ! write(*,*) r02(24), r02(24)
    r03(1) = r03(1) + (zero_m_3(1)*aqx2 - zero_m_2(1)*3.0_dp)*aqx*t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd*0.125_dp*y03
        r03(2) = r03(2) + (zero_m_3(1)*aqx2 - zero_m_2(1))*acy*t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd*0.125_dp*y03
            r03(3) = r03(3) - (zero_m_3(2)*aqx2 - zero_m_2(2))*t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd*0.125_dp*y03
        r03(4) = r03(4) + (zero_m_3(1)*acy2 - zero_m_2(1))*aqx*t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd*0.125_dp*y03
            r03(5) = r03(5) - zero_m_3(2)*aqxy*t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd*0.125_dp*y03
            r03(6) = r03(6) + (zero_m_3(3) - zero_m_2(1))*aqx*t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd*0.125_dp*y03
    r03(7) = r03(7) + (zero_m_3(1)*acy2 - zero_m_2(1)*3.0_dp)*acy*t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd*0.125_dp*y03
            r03(8) = r03(8) - (zero_m_3(2)*acy2 - zero_m_2(2))*t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd*0.125_dp*y03
            r03(9) = r03(9) + (zero_m_3(3) - zero_m_2(1))*acy*t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd*0.125_dp*y03
           r03(10) = r03(10) - (zero_m_3(4) - zero_m_2(2)*3.0_dp)*t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd*0.125_dp*y03
  r03(11) = r03(11) - (zero_m_3(1)*aqx2-zero_m_2(1)*3.0_dp)*aqx * t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd *0.125_dp*(1-y03)
r03(12) = r03(12) - (zero_m_3(1)*aqx2 - zero_m_2(1))*acy*t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd*0.125_dp*(1 - y03)
    r03(13) = r03(13) + (zero_m_3(2)*aqx2 - zero_m_2(2))*t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd*0.125_dp*(1 - y03)
r03(14) = r03(14) - (zero_m_3(1)*acy2 - zero_m_2(1))*aqx*t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd*0.125_dp*(1 - y03)
            r03(15) = r03(15) + zero_m_3(2)*aqxy*t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd*0.125_dp*(1 - y03)
     r03(16) = r03(16) - (zero_m_3(3) - zero_m_2(1))*aqx*t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd*0.125_dp*(1 - y03)
  r03(17) = r03(17) - (zero_m_3(1)*acy2-zero_m_2(1)*3.0_dp)*acy * t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd *0.125_dp*(1-y03)
    r03(18) = r03(18) + (zero_m_3(2)*acy2 - zero_m_2(2))*t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd*0.125_dp*(1 - y03)
     r03(19) = r03(19) - (zero_m_3(3) - zero_m_2(1))*acy*t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd*0.125_dp*(1 - y03)
     r03(20) = r03(20) + (zero_m_3(4) - zero_m_2(2)*3.0_dp)*t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd*0.125_dp*(1 - y03)
            ! do i = 1,20
            !     write(*,*) "r03", r03(i)
            ! enddo
  r04( 1) = r04( 1) +(zero_m_4(1)*aqx2*aqx2-zero_m_3(1)*6.0_dp*aqx2+zero_m_2(1)*3.0_dp)*t_inverse_expon_cd *sq*t_inverse_expon_cd *t_inverse_expon_cd *t_inverse_expon_cd *0.0625_dp
  r04( 2) = r04( 2) +(zero_m_4(1)*aqx2-zero_m_3(1)*3.0_dp)*t_inverse_expon_cd *sq*t_inverse_expon_cd *t_inverse_expon_cd *t_inverse_expon_cd *aqxy*0.0625_dp
  r04( 3) = r04( 3) -(zero_m_4(2)*aqx2-zero_m_3(2)*3.0_dp)*t_inverse_expon_cd *sq*t_inverse_expon_cd *t_inverse_expon_cd *t_inverse_expon_cd *aqx*0.0625_dp
  r04( 4) = r04( 4) +(zero_m_4(1)*aqx2*acy2-zero_m_3(1)*(aqx2+acy2)+zero_m_2(1))*t_inverse_expon_cd *sq*t_inverse_expon_cd *t_inverse_expon_cd *t_inverse_expon_cd *0.0625_dp !this 1.0_dp
  r04( 5) = r04( 5) -(zero_m_4(2)*aqx2-zero_m_3(2))*t_inverse_expon_cd *sq*t_inverse_expon_cd *t_inverse_expon_cd *t_inverse_expon_cd *acy*0.0625_dp
  r04( 6) = r04( 6) +(zero_m_4(3)*aqx2-zero_m_3(1)*aqx2-zero_m_3(3)+zero_m_2(1))*t_inverse_expon_cd *sq*t_inverse_expon_cd *t_inverse_expon_cd *t_inverse_expon_cd *0.0625_dp
  r04( 7) = r04( 7) +(zero_m_4(1)*acy2-zero_m_3(1)*3.0_dp)*t_inverse_expon_cd *sq*t_inverse_expon_cd *t_inverse_expon_cd *t_inverse_expon_cd *aqxy*0.0625_dp
  r04( 8) = r04( 8) -(zero_m_4(2)*acy2-zero_m_3(2))*t_inverse_expon_cd *sq*t_inverse_expon_cd *t_inverse_expon_cd *t_inverse_expon_cd *aqx*0.0625_dp
  r04( 9) = r04( 9) +(zero_m_4(3)-zero_m_3(1))*t_inverse_expon_cd *sq*t_inverse_expon_cd *t_inverse_expon_cd *t_inverse_expon_cd *aqxy*0.0625_dp
  r04(10) = r04(10) -(zero_m_4(4)-zero_m_3(2)*3.0_dp)*t_inverse_expon_cd *sq*t_inverse_expon_cd *t_inverse_expon_cd *t_inverse_expon_cd *aqx*0.0625_dp
  r04(11) = r04(11) +(zero_m_4(1)*acy2*acy2-zero_m_3(1)*6.0_dp*acy2+zero_m_2(1)*3.0_dp)*t_inverse_expon_cd *sq*t_inverse_expon_cd *t_inverse_expon_cd *t_inverse_expon_cd*0.0625_dp
  r04(12) = r04(12) -(zero_m_4(2)*acy2-zero_m_3(2)*3.0_dp)*t_inverse_expon_cd *sq*t_inverse_expon_cd *t_inverse_expon_cd *t_inverse_expon_cd *acy*0.0625_dp
  r04(13) = r04(13) +(zero_m_4(3)*acy2-zero_m_3(1)*acy2-zero_m_3(3)+zero_m_2(1))*t_inverse_expon_cd *sq*t_inverse_expon_cd *t_inverse_expon_cd *t_inverse_expon_cd *0.0625_dp
  r04(14) = r04(14) -(zero_m_4(4)-zero_m_3(2)*3.0_dp)*t_inverse_expon_cd *sq*t_inverse_expon_cd *t_inverse_expon_cd *t_inverse_expon_cd *acy*0.0625_dp
  r04(15) = r04(15) +(zero_m_4(5)-zero_m_3(3)*6.0_dp+zero_m_2(1)*3.0_dp)*t_inverse_expon_cd*sq*t_inverse_expon_cd*t_inverse_expon_cd*t_inverse_expon_cd*0.0625_dp
            ! do i = 1,15
            !     write(*,*) "r04", r04(i)
            ! enddo
          end do !end ket loop
  ...
  eri_value(1) = 3.0_dp*r00(1) + qx*qx*qx*qx*r00(5) + 2*qx*qx*qx*(r01(7) + r01(10)) + 6.0_dp*r02(1) + qx*qx*(r00(2) + 4.0_dp*r00(3) + r00(4) + r02(7) + 4.0_dp*r02(13) + r02(19)) + 2*qx*(3.0_dp*(r01(1) + r01(4)) + r03(1) + r03(11)) + r04(1)
          eri_value(2) = r00(1) + r02(1) + r02(2) + qx*qx*(r00(4) + r02(20)) + 2*qx*(r01(4) + r03(14)) + r04(4)
  eri_value(3) = r00(1) + qx*qx*qz*qz*r00(5) + 2*qx*qz*qz*r01(7) + 2*qx*qx*qz*r01(12) + r02(1) + r02(3) + qz*qz*(r00(2) + r02(7)) + 4*qx*qz*r02(17) + qx*qx*(r00(4) + r02(21)) + 2*qz*(r01(3) + r03(3)) + 2*qx*(r01(4) + r03(16)) + r04(6)
  eri_value(4) = qx*qx*qx*r01(11) + 3.0_dp*r02(4) + qx*qx*(2.0_dp*r02(16) + r02(22)) + qx*(r01(2) + 2.0_dp*r01(5) + r03(2) + 2.0_dp*r03(12)) + r04(2)
  eri_value(5) = qx*qx*qx*qz*r00(5) + qx*qx*qz*(2.0_dp*r01(7) + r01(10)) + qx*qx*qx*r01(12) + 3.0_dp*r02(5) + qx*qz*(r00(2) + 2.0_dp*r00(3) + r02(7) + 2.0_dp*r02(13)) + qx*qx*(2.0_dp*r02(17) + r02(23)) + qz*(3.0_dp*r01(1) + r03(1)) + qx*(r01(3) + 2.0_dp*r01(6) + r03(3) + 2.0_dp*r03(13)) + r04(3)
          eri_value(6) = qx*qx*qz*r01(11) + r02(6) + 2*qx*qz*r02(16) + qx*qx*r02(24) + qz*(r01(2) + r03(2)) + 2*qx*r03(15) + r04(5)
          eri_value(7) = r00(1) + r02(1) + r02(2) + qx*qx*(r00(2) + r02(8)) + 2*qx*(r01(1) + r03(4)) + r04(4)
          eri_value(8) = 3.0_dp*r00(1) + 6.0_dp*r02(2) + r04(11)
          eri_value(9) = r00(1) + r02(2) + r02(3) + qz*qz*(r00(2) + r02(8)) + 2*qz*(r01(3) + r03(8)) + r04(13)
          eri_value(10) = 3.0_dp*r02(4) + qx*(3.0_dp*r01(2) + r03(7)) + r04(7)
          eri_value(11) = r02(5) + qx*qz*(r00(2) + r02(8)) + qz*(r01(1) + r03(4)) + qx*(r01(3) + r03(8)) + r04(8)
          eri_value(12) = 3.0_dp*r02(6) + qz*(3.0_dp*r01(2) + r03(7)) + r04(12)
  eri_value(13) = r00(1) + qx*qx*qz*qz*r00(5) + 2*qx*qx*qz*r01(9) + 2*qx*qz*qz*r01(10) + r02(1) + r02(3) + qx*qx*(r00(2) + r02(9)) + 4*qx*qz*r02(17) + qz*qz*(r00(4) + r02(19)) + 2*qx*(r01(1) + r03(6)) + 2*qz*(r01(6) + r03(13)) + r04(6)
          eri_value(14) = r00(1) + r02(2) + r02(3) + qz*qz*(r00(4) + r02(20)) + 2*qz*(r01(6) + r03(18)) + r04(13)
  eri_value(15) = 3.0_dp*r00(1) + qz*qz*qz*qz*r00(5) + 2*qz*qz*qz*(r01(9) + r01(12)) + 6.0_dp*r02(3) + qz*qz*(r00(2) + 4.0_dp*r00(3) + r00(4) + r02(9) + 4.0_dp*r02(15) + r02(21)) + 2*qz*(3.0_dp*(r01(3) + r01(6)) + r03(10) + r03(20)) + r04(15)
          eri_value(16) = qx*qz*qz*r01(11) + r02(4) + 2*qx*qz*r02(18) + qz*qz*r02(22) + qx*(r01(2) + r03(9)) + 2*qz*r03(15) + r04(9)
  eri_value(17) = qx*qz*qz*qz*r00(5) + qz*qz*qz*r01(10) + qx*qz*qz*(2.0_dp*r01(9) + r01(12)) + 3.0_dp*r02(5) + qx*qz*(r00(2) + 2.0_dp*r00(3) + r02(9) + 2.0_dp*r02(15)) + qz*qz*(2.0_dp*r02(17) + r02(23)) + qx*(3.0_dp*r01(3) + r03(10)) + qz*(r01(1) + 2.0_dp*r01(4) + r03(6) + 2.0_dp*r03(16)) + r04(10)
  eri_value(18) = qz*qz*qz*r01(11) + 3.0_dp*r02(6) + qz*qz*(2.0_dp*r02(18) + r02(24)) + qz*(r01(2) + 2.0_dp*r01(5) + r03(9) + 2.0_dp*r03(19)) + r04(14)
  eri_value(19) = qx*qx*qx*r01(8) + 3.0_dp*r02(4) + qx*qx*(r02(10) + 2.0_dp*r02(16)) + qx*(2.0_dp*r01(2) + r01(5) + 2.0_dp*r03(2) + r03(12)) + r04(2)
          eri_value(20) = 3.0_dp*r02(4) + qx*(3.0_dp*r01(5) + r03(17)) + r04(7)
          eri_value(21) = qx*qz*qz*r01(8) + r02(4) + qz*qz*r02(10) + 2*qx*qz*r02(18) + 2*qz*r03(5) + qx*(r01(5) + r03(19)) + r04(9)
          eri_value(22) = r00(1) + r02(1) + r02(2) + qx*qx*(r00(3) + r02(14)) + qx*(r01(1) + r01(4) + r03(4) + r03(14)) + r04(4)
  eri_value(23) = qx*qx*qz*r01(8) + r02(6) + qx*qz*(r02(10) + r02(16)) + qx*qx*r02(18) + qz*(r01(2) + r03(2)) + qx*(r03(5) + r03(15)) + r04(5)
          eri_value(24) = r02(5) + qx*qz*(r00(3) + r02(14)) + qz*(r01(1) + r03(4)) + qx*(r01(6) + r03(18)) + r04(8)
  eri_value(25) = qx*qx*qx*qz*r00(5) + qx*qx*qx*r01(9) + qx*qx*qz*(r01(7) + 2.0_dp*r01(10)) + 3.0_dp*r02(5) + qx*qx*(r02(11) + 2.0_dp*r02(17)) + qx*qz*(2.0_dp*r00(3) + r00(4) + 2.0_dp*r02(13) + r02(19)) + qz*(3.0_dp*r01(4) + r03(11)) + qx*(2.0_dp*r01(3) + r01(6) + 2.0_dp*r03(3) + r03(13)) + r04(3)
          eri_value(26) = r02(5) + qx*qz*(r00(4) + r02(20)) + qz*(r01(4) + r03(14)) + qx*(r01(6) + r03(18)) + r04(8)
  eri_value(27) = qx*qz*qz*qz*r00(5) + qz*qz*qz*r01(7) + qx*qz*qz*(r01(9) + 2.0_dp*r01(12)) + 3.0_dp*r02(5) + qz*qz*(r02(11) + 2.0_dp*r02(17)) + qx*qz*(2.0_dp*r00(3) + r00(4) + 2.0_dp*r02(15) + r02(21)) + qz*(2.0_dp*r01(1) + r01(4) + 2.0_dp*r03(6) + r03(16)) + qx*(3.0_dp*r01(6) + r03(20)) + r04(10)
  eri_value(28) = qx*qx*qz*r01(11) + r02(6) + qx*qx*r02(18) + qx*qz*(r02(16) + r02(22)) + qz*(r01(5) + r03(12)) + qx*(r03(5) + r03(15)) + r04(5)
  eri_value(29) = r00(1) + qx*qx*qz*qz*r00(5) + qx*qz*qz*(r01(7) + r01(10)) + qx*qx*qz*(r01(9) + r01(12)) + r02(1) + r02(3) + qz*qz*(r00(3) + r02(13)) + qx*qx*(r00(3) + r02(15)) + qx*qz*(r02(11) + 2.0_dp*r02(17) + r02(23)) + qz*(r01(3) + r01(6) + r03(3) + r03(13)) + qx*(r01(1) + r01(4) + r03(6) + r03(16)) + r04(6)
  eri_value(30) = qx*qz*qz*r01(11) + r02(4) + qz*qz*r02(16) + qx*qz*(r02(18) + r02(24)) + qz*(r03(5) + r03(15)) + qx*(r01(5) + r03(19)) + r04(9)
          eri_value(31) = qx*qx*qz*r01(8) + r02(6) + qx*qx*r02(12) + 2*qx*qz*r02(16) + 2*qx*r03(5) + qz*(r01(5) + r03(12)) + r04(5)
          eri_value(32) = 3.0_dp*r02(6) + qz*(3.0_dp*r01(5) + r03(17)) + r04(12)
  eri_value(33) = qz*qz*qz*r01(8) + 3.0_dp*r02(6) + qz*qz*(r02(12) + 2.0_dp*r02(18)) + qz*(2.0_dp*r01(2) + r01(5) + 2.0_dp*r03(9) + r03(19)) + r04(14)
          eri_value(34) = r02(5) + qx*qz*(r00(3) + r02(14)) + qx*(r01(3) + r03(8)) + qz*(r01(4) + r03(14)) + r04(8)
  eri_value(35) = qx*qz*qz*r01(8) + r02(4) + qz*qz*r02(16) + qx*qz*(r02(12) + r02(18)) + qx*(r01(2) + r03(9)) + qz*(r03(5) + r03(15)) + r04(9)
          eri_value(36) = r00(1) + r02(2) + r02(3) + qz*qz*(r00(3) + r02(14)) + qz*(r01(3) + r01(6) + r03(8) + r03(18)) + r04(13)
          ! do i = 1,36
  ...
  eri_value(36)=buff(11)*buff(12)*sqrt3*trans(1) + buff(8)*buff(9)*sqrt3*trans(2) + buff(2)*buff(3)*sqrt3*trans(3) + (buff(12)*buff(8) + buff(11)*buff(9))*sqrt3*trans(4) + (buff(12)*buff(2) + buff(11)*buff(3))*sqrt3*trans(5) + (buff(3)*buff(8) + buff(2)*buff(9))*sqrt3*trans(6)
      ...
      ! write(*,*) "time in 0022 kernel", kernel_only2-kernel_only1
    end do !tiles
    kernel_full2 = omp_get_wtime()
    ! write(*,*) "time in 0022 full", kernel_full2-kernel_full1

    deallocate (n00bra)
    deallocate (xint00bra)
    deallocate (n22ket)
    deallocate (xint22ket)
  end subroutine int0022
end submodule
